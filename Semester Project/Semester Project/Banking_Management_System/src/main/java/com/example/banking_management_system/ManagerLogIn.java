...
public class ManagerLogIn implements Initializable {
    ...
    void onSignInClicked(ActionEvent event) throws ClassNotFoundException, SQLException, IOException {
        ...
        ResultSet rs;
        PreparedStatement stat = dataBaselink.prepareStatement(("SELECT * FROM manager"));
        rs = stat.executeQuery();

        boolean isValidUser = false;

        while (rs.next()) {
            if (Objects.equals(rs.getString("managerName"), Username_field.getText())) {
                if (Objects.equals(rs.getString("managerpass"), Password_field.getText())) {
                    isValidUser = true;
                    break;
                }
            }
