    ...
    void onWithdrawButtonClicked(ActionEvent event) throws ClassNotFoundException, SQLException {
        ...
        if (resultSet.next()) {
            ...
            if (amount > currentDeposit) {
                // Show an error message if the withdrawal amount is greater than the available balance
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Error");
                alert.setHeaderText("Insufficient Balance");
                alert.setContentText("The withdrawal amount is greater than the available balance.");
                alert.showAndWait();
            ...
            alert.setContentText("Withdraw Amount successfully!");
            alert.showAndWait();
